			...
			if($i % 5 == 0) {
				...
				if (!empty($products)) {
					foreach ($products as $product) {
						$csv_data = array($product['asin'],   $product['new_lowest'], $product['new_2lowest'], $product['used_lowest'], $product['used_2lowest']);
						fputcsv($fh_w, $csv_data);
					}
				}
				$temp_arr = array();
...
//                        if (strcasecmp($xml3->item_condition,"New")!=0 ) {
//				continue;
//			}
...
//                                continue;
//                        }
//                         if ($xml3->Qualifiers->ItemCondition == 'Used') {
//                             continue;
//                         }
			...
			$re_arr[$i]['landed_amount'] = $landed_amount;
                        ...
                        echo $re_arr[$i]['asin']."\t";
			echo $re_arr[$i]['item_condition']."\t";
                        echo $re_arr[$i]['fulfillment_channel']."\t";
			echo $re_arr[$i]['ships_domestically']."\t";
                        echo $re_arr[$i]['shipping_time']."\t";
                        echo $re_arr[$i]['seller_positive_feedback_rating']."\t";
                        echo $re_arr[$i]['seller_feedback_count']."\t";
                        echo $re_arr[$i]['landed_amount']."\n";
				...
				$a[$i]++;
                         ...
                         if ($xml3->Qualifiers->ItemCondition == 'Used') {
                             $b[$i]++;
                              if($b[$i]==2&&$re_arr[$i]['used_2lowest']==null){
                                $landed_amount = $xml3->Price->LandedPrice->Amount;
                                $re_arr[$i]['used_2lowest'] = $landed_amount;
                              }
                            
			}
                        if($xml3->Qualifiers->ItemCondition == 'New'&&$re_arr[$i]['new_lowest']==null){
                            $landed_amount = $xml3->Price->LandedPrice->Amount;
                            $re_arr[$i]['new_lowest'] = $landed_amount;
                        }
                        if($xml3->Qualifiers->ItemCondition == 'Used'&&$re_arr[$i]['used_lowest']==null){
                            $landed_amount = $xml3->Price->LandedPrice->Amount;
